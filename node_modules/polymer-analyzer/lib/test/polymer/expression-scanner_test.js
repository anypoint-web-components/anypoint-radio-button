...
suite('ExpressionScanner', () => {
    suite('scanning html for expressions', () => {
      ...
      `;
            const underliner = test_utils_1.CodeUnderliner.withMapping(test_utils_1.resolvedUrl `test.html`, contents);
            ...
            chai_1.assert.deepEqual(generalExpressions.map((e) => e.databindingInto), [
                'attribute',
                'attribute',
                'attribute',
                'attribute',
          ...
          <div id="{{kebab.case-path}}"></div>
            ...
            chai_1.assert.deepEqual(expressions.map((e) => e.eventName), [
                undefined,
                'changed',
                undefined,
                undefined,
            ...
            chai_1.assert.deepEqual(expressions.map((e) => e.attribute && e.attribute.name), ['id', 'value', 'id', 'id', 'id', 'id', 'id']);
            chai_1.assert.deepEqual(expressions.map((e) => e.properties.map((p) => p.name)), [
                ['foo'],
                ['val'],
                ['bada', 'wing', 'daba'],
                ['bar'],
            ...
            chai_1.assert.deepEqual(expressions.map((e) => e.isCompleteBinding), [true, true, true, true, true, true, true]);
        }));
...
~~~~~~~~~~~~`
    ...
    suite('parsing expressions from javascript string literals', () => {
      ...
      `;
            const underliner = test_utils_1.CodeUnderliner.withMapping(test_utils_1.resolvedUrl `test.html`, contents);
            const javascriptDocument = new javascript_parser_1.JavaScriptParser().parse(contents, test_utils_1.resolvedUrl `test.html`, new package_url_resolver_1.PackageUrlResolver());
            const literals = javascriptDocument.ast.program
