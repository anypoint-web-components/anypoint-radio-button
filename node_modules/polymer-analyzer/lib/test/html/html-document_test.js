...
suite('ParsedHtmlDocument', () => {
    ...
    const file = fs.readFileSync(path.join(test_utils_1.fixtureDir, url), 'utf8');
    const analyzer = analyzer_1.Analyzer.createForDirectory(test_utils_1.fixtureDir);
    const document = parser.parse(file, analyzer.resolveUrl(url), new package_url_resolver_1.PackageUrlResolver({ packageDir: test_utils_1.fixtureDir }));
    ...
    suite('sourceRangeForNode()', () => {
        test('works for comments', () => __awaiter(this, void 0, void 0, function* () {
            const comments = [...dom5.depthFirst(document.ast)].filter(parse5.treeAdapters.default.isCommentNode);
...
~~~~`);
    ...
    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~`);
        ...
        test(testName, () => __awaiter(this, void 0, void 0, function* () {
            ...
            const document = parser.parse(yield analyzer.load(url), url, new package_url_resolver_1.PackageUrlResolver({ packageDir: test_utils_1.fixtureDir }));
...
~~~~~~~`);
    ...
    suite('sourceRangeForAttribute', () => {
        let complexTags;
        setup(() => {
            complexTags = [...dom5.queryAll(document.ast, dom5.predicates.hasTagName('complex-tag'))];
            chai_1.assert.equal(complexTags.length, 1);
        });
