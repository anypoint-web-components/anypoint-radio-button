...
class BehaviorVisitor {
    ...
    enterVariableDeclaration(node, _parent, path) {
        ...
        this._initBehavior(node, ast_value_1.getIdentifierName(node.declarations[0].id), path);
    }
    /**
    ...
    _initBehavior(node, name, path) {
        ...
        this.propertyHandlers =
            declaration_property_handlers_1.declarationPropertyHandlers(behavior, this.document, path.scope);
        ...
        this.propertyHandlers = declaration_property_handlers_1.declarationPropertyHandlers(this.currentBehavior, this.document, path.scope);
    ...
    _parseChainedBehaviors(node, scope) {
        ...
        if (expression && babel.isArrayExpression(expression)) {
            for (const arrElement of expression.elements) {
                ...
                if (behaviorName) {
                    chained.push(new model_1.ScannedReference('behavior', behaviorName, this.document.sourceRangeForNode(arrElement), arrElement, scope));
                }
            }
            if (chained.length > 0) {
