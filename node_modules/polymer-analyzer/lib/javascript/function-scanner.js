...
class FunctionVisitor {
    ...
    enterFunctionDeclaration(node, _parent) {
        this._initFunction(node, ast_value_1.getIdentifierName(node.id));
    }
    /**
     * Scan object method declarations.
     */
    enterObjectMethod(node, _parent) {
        this._initFunction(node, ast_value_1.getIdentifierName(node.key));
    }
    /**
    ...
    enterVariableDeclaration(node, _parent) {
        ...
        if (declarationValue && babel.isFunction(declarationValue)) {
            this._initFunction(declarationValue, ast_value_1.getIdentifierName(declaration.id), node);
        }
    }
    /**
    ...
    enterAssignmentExpression(node, parent) {
        if (babel.isFunction(node.right)) {
            this._initFunction(node.right, ast_value_1.getIdentifierName(node.left), parent);
        }
    }
    /**
    ...
    _initFunction(node, analyzedName, docNode) {
        if (docNode === undefined) {
            docNode = node;
        }
        ;
        const docs = jsdoc.parseJsdoc(esutil_1.getAttachedComment(docNode) || '');
        ...
        this.functions.add(new function_1.ScannedFunction(functionName, description, summary, esutil_1.getOrInferPrivacy(specificName, docs), node, docs, sourceRange, functionParams, functionReturn, templateTypes));
    }
}
