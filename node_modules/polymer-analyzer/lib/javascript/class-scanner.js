...
class ClassScanner {
    ...
    _makeElementFeature(element, document) {
        ...
        else if (babel.isClassExpression(astNode) || babel.isClassDeclaration(astNode)) {
...
class PrototypeMemberFinder {
    ...
    _createMemberFromMemberExpression(node, jsdocAnn) {
        ...
        if (jsdoc.hasTag(jsdocAnn, 'function')) {
            const prop = this._createMethodFromExpression(node.property.name, node, jsdocAnn);
            if (prop) {
                this._addMethodToClass(cls, prop);
            }
        }
        else {
            const method = this._createPropertyFromExpression(node.property.name, node, jsdocAnn);
            if (method) {
                this._addPropertyToClass(cls, method);
            }
        }
...
class ClassFinder {
    ...
    enterAssignmentExpression(node, parent, path) {
        this.handleGeneralAssignment(astValue.getIdentifierName(node.left), node.right, node, parent, path);
    }
    enterVariableDeclarator(node, parent, path) {
        if (node.init) {
            this.handleGeneralAssignment(astValue.getIdentifierName(node.id), node.init, node, parent, path);
        }
    }
    /** Generalizes over variable declarators and assignment expressions. */
    handleGeneralAssignment(assignedName, value, assignment, statement, path) {
    ...
    _classFound(name, doc, astNode, scope) {
        ...
        this.classes.push(new model_1.ScannedClass(namespacedName, name, astNode, doc, (doc.description || '').trim(), this._document.sourceRangeForNode(astNode), properties, methods, esutil_1.getStaticMethods(astNode, this._document), this._getExtends(astNode, doc, warnings, this._document, scope), jsdoc.getMixinApplications(this._document, astNode, doc, warnings, scope), esutil_1.getOrInferPrivacy(namespacedName || '', doc), warnings, jsdoc.hasTag(doc, 'abstract'), jsdoc.extractDemos(doc)));
     ...
     */
    _getExtends(node, docs, warnings, document, scope) {
        ...
        if (extendsAnnotations.length > 0) {
            ...
            if (extendsId == null) {
                ...
                return new model_1.ScannedReference('class', extendsId, sourceRange, undefined, scope);
            }
        }
            ...
            if (superClass != null) {
                ...
                if (extendsId != null) {
                    ...
                    return new model_1.ScannedReference('class', extendsId, sourceRange, node.superClass, scope);
                }
            }
        }
    }
...
function extractPropertiesFromClass(astNode, document) {
    ...
    const construct = astNode.body.body.find((member) => babel.isClassMethod(member) &&
...
function extractPropertyFromExpressionStatement(statement, document) {
    ...
    if (babel.isAssignmentExpression(statement.expression)) {
        ...
        defaultValue = babel_generator_1.default(statement.expression.right).code;
    }
